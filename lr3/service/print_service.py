...
def access_print_service(ticket_id, ticket_hash, username, session_key):
    if verify_service_ticket(ticket_id, ticket_hash, username, "printservice", session_key):
        return f"Access granted to printservice for {username} (Session Key: {session_key})"
